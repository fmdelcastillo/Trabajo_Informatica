...
void Enemigo::refresh(float t,vector3D ori) {
	...
	vector3D v;
	float ganancia = 0.2; //regula la velocidad

	//v.x = position.x - ori.x;
	//v.z = position.z - ori.z;
	


	float orientacion = atan2(v.x, v.z);
	
		
		//position.x += speed * cos(orientacion * ganancia) * t;
		//position.z += -speed * sin(orientacion * ganancia) * t;
	position.x = position.x-(position.x - ori.x) * ganancia*t;
	position.z = position.z-(position.z - ori.z) * ganancia*t;
